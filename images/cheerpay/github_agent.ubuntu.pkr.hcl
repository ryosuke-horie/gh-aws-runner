...
# GitHub Runnerのバージョン指定 2024/09/03現在最新バージョンは2.320.0
...
# セキュリティグループIDの指定
# 手動作成したpacker-builder-vpcのセキュリティグループIDを指定
variable "security_group_id" {
  description = "The ID of the security group Packer will associate with the builder to enable access"
  type        = string
  default     = "sg-0efe25041bdde04e5"
}

# サブネットIDの指定
# 手動作成したpacker-builder-vpcのパブリックサブネットIDを指定
variable "subnet_id" {
  description = "If using VPC, the ID of the subnet, such as subnet-12345def, where Packer will launch the EC2 instance. This field is required if you are using an non-default VPC"
  type        = string
  default     = "subnet-0b4c193701b427502"
}
...
source "amazon-ebs" "githubrunner" {
  ami_name                                  = "cheerpay-github-runner-ubuntu-jammy-amd64-${formatdate("YYYYMMDDhhmm", timestamp())}"
...
build {
  ...
  inline = concat([
    ...
    "sudo -u postgres psql -c \"CREATE DATABASE cheerpay_test OWNER laravel;\"",
